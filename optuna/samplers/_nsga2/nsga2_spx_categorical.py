...
class NSGAIISPXCategoricalSampler(BaseSampler):
    ...
    def __init__(
        ...
        *,
        ...
        if n <= 0 or n >= population_size:
            raise ValueError("select n size")
    ...
    def sample_relative(
        ...
        if parent_generation >= 0:
            ...
            for param_name, param_distribution in search_space.items():
                if isinstance(param_distribution, CategoricalDistribution):
                    if self._rng.rand() <= self._swapping_prob:
                        child[param_name] = parents[self._n - 1].params[param_name]
                    else:
            ...
            count = 0
            while True:
                _params = self.spx(xs, self._epsilon)
    ...
    def spx(
        ...
        rs = [np.power(self._rng.uniform(0, 1), 1 / (k + 1)) for k in range(self._n)]

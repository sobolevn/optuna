...
class NSGAIIvSBXCategoricalSampler(BaseSampler):
    ...
    def sample_relative(
        ...
        if parent_generation >= 0:
            ...
            if self._rng.rand() < self._crossover_prob:
                ...
                p1 = p0
            # print("sample relative", trial_id)
    ...
    def vsbx(self, x1: np.float64, x2: np.float64, eta: float) -> np.ndarray:
        # https://core.ac.uk/download/pdf/230075112.pdf
        r = self._rng.uniform(0, 1, size=x1.size)
        beta_1i = np.power(1 / 2 * r, 1 / (eta + 1))
        beta_2i = np.power(1 / 2 * (1 - r), 1 / (eta + 1))
        if r <= 0.5:
            c1 = 0.5 * ((1 + beta_1i) * x1 + (1 - beta_1i) * x2)
        else:
            c1 = 0.5 * ((1 - beta_1i) * x1 + (1 + beta_1i) * x2)
        if r <= 0.5:
            c2 = 0.5 * ((3 - beta_2i) * x1 - (1 - beta_2i) * x2)
        else:
            c2 = 0.5 * (-(1 - beta_2i) * x1 + (3 - beta_2i) * x2)
        ...
        if param is None or self._rng.rand() < self._swapping_prob:
            if isinstance(param_distribution, CategoricalDistribution):
                ...
                while True:
                    ...
                    param = trans.untransform(param)
                    param = param.get(param_name, None)
                    if param_distribution._contains(param):
                        break
                    if count >= self._max_resampling_count:
                        param = np.clip(
                            param, param_distribution.low, param_distribution.high
                        )
                        break
                    count += 1
                parent_params_len = len(p1.params)
